...
<Anchor id="reconciliation-demo" />
**Try it:** hold WASD. The blue square is your predicted movement. The red square is server truth (delayed). “Snap” shows why raw correction jitters; “smooth” shows why we blend.
...
**Try it:** move with WASD, then mess with tick rate/jitter/loss. You’ll see remote motion go from smooth → steppy → hitchy, even though your local motion stays responsive.
...
- **Interpolation buffer:** Buffering 2-3 snapshots smooths network jitter so remote players don't stutter. We lerp directly to the latest position — simpler, but more sensitive to jitter. <PerplexityLink query="snapshot interpolation buffer multiplayer games" />
...
- **Input-based prediction:** Sending inputs (not positions) lets the server simulate authoritatively — prevents cheating, enables correction. We send positions directly, which is faster to build but not secure. <PerplexityLink query="client side prediction server reconciliation multiplayer" />

The “correction” part we intentionally skipped in POC 0 lives up in the theory section now: [Prediction + reconciliation demo](#reconciliation-demo).
...
- Add an interpolation buffer to smooth network jitter
- Build a debug overlay showing RTT, jitter, and update rate
