...
Latency is one of those things you can understand intellectually and still underestimate. **Try it for yourself** — click the demo, hold WASD, and flip between 0ms, 50ms, 100ms, 150ms, 200ms.
...
<Alert title="Runnable implementation (what this repo actually ships)">

The code snippets below teach the ideas in small steps, but the runnable POC is the “final form”:

- **Server** runs a fixed 20Hz tick via PartyKit **alarms** (`room.storage.setAlarm` + `onAlarm`) and broadcasts **snapshots**.
- **Client → server** sends **inputs** (`{ type: \"input\", seq, dt, keys }`), not transforms.
- **Client prediction** moves immediately.
- **Server reconciliation** snaps to the authoritative snapshot, then replays un-acked inputs.
- **Remote interpolation** uses a small snapshot buffer and renders ~100ms behind.

Code pointers:
- `party/main.ts`
- `app/play/0-multiplayer-foundation/use-multiplayer.ts`
- `app/play/0-multiplayer-foundation/local-player.tsx`
- `app/play/0-multiplayer-foundation/remote-player.tsx`
...
export default {
  ...
  async onAlarm(room) {
    const state = (await room.storage.get("state")) ?? { players: {}, tick: 0 }
    if (Object.keys(state.players).length === 0) return

    // Advance authoritative sim at fixed dt, then broadcast a snapshot.
    // (In this repo, we broadcast { type: "snapshot", tick, serverTime, players: [...] }.)
...
<Alert title="POC 0 implementation: input-based sync with server reconciliation">

The runnable POC implements **input-based sync** — clients send inputs (WASD keys) with sequence numbers, and the server runs an authoritative simulation at 20Hz. Clients predict locally and reconcile when server snapshots arrive.

This is still a basic implementation: no collisions, no lag compensation, no client-side smoothing on correction beyond simple snap/lerp. But it demonstrates the core techniques: server authority, client-side prediction, and server reconciliation.
...
export function MultiplayerScene({ roomId }: { roomId: string }) {
  const { connected, playerId, players, sendInput } = useMultiplayer(roomId)
...
- No extrapolation when packets arrive late.
...
This POC is complete. The foundation works — two players move smoothly in a shared space.
