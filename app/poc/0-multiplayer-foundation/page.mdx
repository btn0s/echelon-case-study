...
Notice that we're mutating the player object in the `player-moved` handler (`player.position = data.position`). This is safe because we're mutating an object inside a Map that we just created, not the previous state. React will still detect the change because we're returning a new Map reference.
