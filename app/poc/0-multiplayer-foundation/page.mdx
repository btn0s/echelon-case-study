...
Before we build, we’re going to earn a bit of context: latency, authority, and the tradeoffs that let co-op feel immediate without clients disagreeing about what’s true.
...
- Wait for the host to confirm, and movement feels sluggish
...
But this is co-op. We don’t need competitive-grade fairness guarantees. We need one authority that keeps the session consistent, and movement that feels immediate. <PerplexityLink query="why host auth is fine for coop and why you should switch to dedicated for pvp" />

**For Echelon, host-authoritative is the right default.** It gives us one source of truth without standing up dedicated infrastructure. The tradeoff is that we accept a single point of failure: the host’s internet/CPU affects everyone, and if they leave the session ends. Luckily, there are common techniques like host migration to mitigate this. <PerplexityLink query="host migration listen server co-op games how it works" />

<Anchor id="the-key-concepts" />
## The Key Concepts

Now that we've chosen host-authoritative, everything else is three techniques applied in the right places.
...
In host-authoritative, the host's machine owns critical game state — health, inventory, objectives, AI positions. If a client says "I completed the objective," the host ignores it. The host decides what's true.

"Authoritative" means one machine resolves conflicts — it doesn't have to be a cloud server. For co-op, the host's machine is sufficient.

But if the host owns *everything*, movement feels laggy for remote players. So we let clients predict some things locally — their own movement, visual effects. The host remains authoritative and can correct, but the client doesn't wait.

This split is the foundation: **host authoritative for critical state, client predictive for responsiveness.**
...
Remember the paradox: wait for host confirmation, and movement feels laggy. Show movement immediately, and we might be wrong. **Prediction** solves this. <PerplexityLink query="client side prediction multiplayer games how it works" />

Press forward → you move immediately → host confirms later. The client doesn't wait. If the host says "actually, you collided with something," the client corrects smoothly — sliding to the correct position rather than snapping (snapping causes visible jitter).
...
You also can't update their position every frame (60 times per second) — that's too much bandwidth. Instead, the host sends "snapshots" of game state periodically, maybe 20-30 times per second. Between snapshots, clients **interpolate** — smoothly blending between the last known positions.
...
We’ve named the hard parts: latency, a single authority, and the two tricks that keep co-op feeling good (predict yourself, interpolate everyone else).

Now that we understand the tradeoffs, we can move on to building it.
