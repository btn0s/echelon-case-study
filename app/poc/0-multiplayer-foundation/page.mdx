...
When you press "move forward" on your keyboard, light travels through wires to a server, the server processes your input, and light travels back. Even at the speed of light, this takes time — typically 20-150ms depending on distance. [dive deeper →](https://www.perplexity.ai/search?q=network+latency+in+multiplayer+games+explained)
...
Fighting games pioneered rollback netcode (predicting inputs rather than waiting for confirmation), and libraries like GGPO made this accessible. While overkill for our co-op game, understanding rollback helps appreciate the tradeoff space. [dive deeper →](https://www.perplexity.ai/search?q=GGPO+rollback+netcode+explained+fighting+games)
...
There are three fundamental approaches to multiplayer networking. Each makes different tradeoffs between complexity, consistency, and responsiveness. [dive deeper →](https://www.perplexity.ai/search?q=peer+to+peer+vs+client+server+multiplayer+game+architecture)

### Option 1: Peer-to-Peer (P2P)

Every player's machine talks directly to every other player's machine. No central server. [dive deeper →](https://www.perplexity.ai/search?q=peer+to+peer+networking+in+games+pros+cons)
...
**Cons:**
- N² connections (scales poorly)
- No authoritative state (who's right when players disagree?)
- Cheating is trivial (your machine decides what's true)
- NAT traversal is painful [dive deeper →](https://www.perplexity.ai/search?q=NAT+traversal+hole+punching+multiplayer+games)

**Used by:** Older fighting games, some indie games, local multiplayer

### Option 2: Host-Authoritative (Listen Server)

One player's machine acts as both player and server. Other players connect to them. [dive deeper →](https://www.perplexity.ai/search?q=listen+server+host+migration+multiplayer+games)
...
- Simpler than full client-server
- Works for small player counts

**Cons:**
- Host has zero latency (unfair advantage in competitive games)
- If host disconnects, game ends (or requires migration)
- Host's internet quality affects everyone

**Used by:** Payday 2, Deep Rock Galactic, many co-op games

### Option 3: Client-Server (Dedicated Server)

A dedicated server owns the game state. All players are equal clients. [dive deeper →](https://www.perplexity.ai/search?q=dedicated+server+architecture+multiplayer+games)
...
- Server can validate/prevent cheating
- Server can persist state, handle reconnects
...
<Tradeoffs title="Which approach for Echelon?">
...
For development, we'll use a simple WebSocket server. For production, we could use [Hathora](https://hathora.dev/), [Colyseus Cloud](https://colyseus.io/), or self-host. [dive deeper →](https://www.perplexity.ai/search?q=game+server+hosting+options+hathora+colyseus+2024)
...
**Authority** means "who is the source of truth for this piece of state?" [dive deeper →](https://www.perplexity.ai/search?q=server+authority+vs+client+authority+multiplayer+games)
...
For responsive interactions, the client can be authoritative (with server validation):
- Player movement (predict locally, server corrects)
...
[dive deeper on Perplexity →](https://www.perplexity.ai/search?q=why+never+trust+the+client+game+security+cheating)

</Pitfall>

### Concept 2: Client-Side Prediction

**Prediction** means the client immediately shows the result of player actions without waiting for server confirmation. [dive deeper →](https://www.perplexity.ai/search?q=client+side+prediction+multiplayer+games+how+it+works)
...
**The input buffer pattern:** [dive deeper →](https://www.perplexity.ai/search?q=input+buffer+client+prediction+server+reconciliation)
...
**Interpolation** is how we make other players (and networked objects) move smoothly despite only receiving updates 20-30 times per second. [dive deeper →](https://www.perplexity.ai/search?q=interpolation+vs+extrapolation+multiplayer+games+networking)
...
Left 4 Dead 2's Source engine runs the server at 30 ticks per second for most gameplay. Clients receive snapshots and interpolate between them. [dive deeper →](https://www.perplexity.ai/search?q=left+4+dead+2+source+engine+networking+tick+rate)
...
We're building with the pmndrs (Poimandres) ecosystem — React Three Fiber for 3D rendering, with a WebSocket-based networking layer. [dive deeper →](https://www.perplexity.ai/search?q=pmndrs+poimandres+react+three+fiber+ecosystem)

### Core Technologies

**React Three Fiber (R3F)**
The React renderer for Three.js. Lets us build 3D scenes with React components. We'll use this for all game rendering. [dive deeper →](https://www.perplexity.ai/search?q=react+three+fiber+vs+vanilla+threejs+pros+cons)

**Zustand**
Minimal state management from the pmndrs ecosystem. R3F uses it internally, and we'll use it for local game state. Important: Zustand is client-side only — we need networking to sync between clients. [dive deeper →](https://www.perplexity.ai/search?q=zustand+state+management+react+three+fiber)

**WebSockets**
The transport layer for real-time communication. We have options for the server: [dive deeper →](https://www.perplexity.ai/search?q=websocket+vs+webrtc+for+multiplayer+games)
...
**For this POC, we'll use PartyKit** — it's the fastest way to get a WebSocket server running, deploys to the edge (low latency), and has excellent DX. [dive deeper →](https://www.perplexity.ai/search?q=partykit+websocket+edge+deployment+tutorial)
...
PartyKit gives us a WebSocket server that deploys to Cloudflare's edge network. [dive deeper →](https://www.perplexity.ai/search?q=partykit+cloudflare+workers+websocket+server+setup)
...
export default {
  ...
  onConnect(connection: PartyConnection, room) {
    const state = room.storage.get("state") as GameState ?? { players: new Map() }
    ...
    room.broadcast(JSON.stringify({
      type: "player-joined",
      player: state.players.get(connection.id),
    }), [connection.id])
  ...
  onMessage(message: string, connection: PartyConnection, room) {
    const data = JSON.parse(message)
    const state = room.storage.get("state") as GameState
    
    if (data.type === "move") {
      // Server validates and updates authoritative position
      const player = state.players.get(connection.id)
      if (player) {
  ...
  onClose(connection: PartyConnection, room) {
    const state = room.storage.get("state") as GameState
    state.players.delete(connection.id)
...
A React hook to manage the WebSocket connection and state. [dive deeper →](https://www.perplexity.ai/search?q=react+websocket+hook+pattern+real+time+state)
...
The React Three Fiber scene that renders all players. [dive deeper →](https://www.perplexity.ai/search?q=react+three+fiber+multiplayer+scene+rendering+players)
...
The local player predicts movement immediately, then sends to server. [dive deeper →](https://www.perplexity.ai/search?q=client+side+prediction+implementation+javascript)
...
Remote players interpolate smoothly between network updates. [dive deeper →](https://www.perplexity.ai/search?q=lerp+interpolation+smooth+movement+game+networking)
...
2. **Update frequency:** Is 20Hz enough, or do we need higher? [dive deeper →](https://www.perplexity.ai/search?q=tick+rate+update+frequency+multiplayer+games+optimal)
...
- **POC 1 (Destruction):** How do we sync physics events? [dive deeper →](https://www.perplexity.ai/search?q=syncing+physics+destruction+multiplayer+games)
...
We need to measure what matters: [dive deeper →](https://www.perplexity.ai/search?q=measuring+network+latency+jitter+multiplayer+games)
...
- Should we use delta compression or full snapshots? [dive deeper →](https://www.perplexity.ai/search?q=delta+compression+vs+full+snapshots+game+networking)
- How do we handle the first few frames before interpolation has data?
- What's the right smoothing factor for interpolation?
- Should we add extrapolation (predicting where remote players are going)? [dive deeper →](https://www.perplexity.ai/search?q=extrapolation+dead+reckoning+multiplayer+games)
