...
### Solution to Latency: Local simulation

Once you have authority, you hit the second problem: you can’t wait for the network on every input.

So clients run a **local simulation**: a fast, best-effort version of the world used for responsiveness. It’s allowed to be wrong, as long as the authority can correct it without breaking trust.

The boundary is simple:

- **Safe to simulate locally**: things that primarily affect *your* feel (your movement, camera, VFX/UI)
- **Not safe to simulate locally as truth**: shared state other players coordinate on (objectives, heat, AI decisions, destruction)

Two techniques cover most games: predict yourself, interpolate everyone else.

#### Technique 1: Local prediction (you)
...
#### Technique 2: Remote interpolation (everyone else)
...
<AccordionSection title="Where we draw the trust boundary in Echelon" value="trust-boundary">

The authority owns shared state that affects coordination:
...
Clients simulate locally for responsiveness where the authority can later correct safely:
